...
def a_star(start_board, metric):
    ...
    while len(priority_queue) != 0:
        ...
        while current_element.heap_activity != True:
            ...
            if len(priority_queue) == 0:
                ...
                return False, str(-1), str(-1), str(len(visited_elements) + len(priority_queue)), str(len(priority_queue)), str(my_max_depth), str(my_time)
            current_element = heapq.heappop(priority_queue)
        ...
        if mf.is_goal(current_element):
            ...
            my_time = end - start
            return True, current_element.history, str(len(current_element.history)), str(len(visited_elements) + len(priority_queue)), str(len(priority_queue)), str(my_max_depth), str(my_time)
    ...
    my_time = end - start
    # false, -1, -1, visited elements, processed elements, max depth, time
    return False, str(-1), str(-1), str(len(visited_elements)), str(len(priority_queue) + len(priority_queue)), str(my_max_depth), str(my_time)
